...
def speed_test(metadata, cfg):
    ...
    times = [d["time"] for d in metadata]
    stations = [d["station"] for d in metadata]
    lats = [x for _, x in sorted(zip(times, lats), key=lambda x: x[0])]
    lons = [x for _, x in sorted(zip(times, lons), key=lambda x: x[0])]
    stations = [x for _, x in sorted(zip(times, stations), key=lambda x: x[0])]
    ...
    for idx in range(len(times) - 1):
        ...
        if speed > cfg:
            print(
                "WARNING: Navigation too fast between "
                + stations[idx]
                + " and "
                + stations[idx + 1]
            )
