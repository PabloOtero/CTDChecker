...
def _parse_cnv_medatlas(file):
    ...
    for irow, line in enumerate(lines):
        ...
        if irow == 1:
            ...
            [day, month, year] = start_date_of_cruise.split("-")
            if len(day) == 2:
                start_date_of_cruise = year + "-" + month + "-" + day
            elif len(day) == 4:
                start_date_of_cruise = day + "-" + month + "-" + year
            else:
                sys.exit("ERROR. Bad format in start date of cruise in header of CSR file")
        ...
        if "Availability=" in line:
            ...
            if dataset_access == "L":
                dataset_access = "RS"
        if "EDMO_CODE=" in line:
            edmo_code = line.split("EDMO_CODE=")[1].strip()
    ...
    medatlas.update(
        {
            "edmo_author": edmo_code,
            "edmo_originator": edmo_code,
            "edmo_custodian": edmo_code,
            "edmo_distributor": edmo_code,
            "area_type": "Point",
...
def create_cdi_from_medatlas(file, outpath_cdi, csr):
    """
       ...
       Input: - Data file in medatlas format
              ...
              - Info from CSR file, obtained from a previous parsing:
    ...
    #dom_CSR = ET.parse(pkg_resources.resource_string("ctd", CSR_LOCAL_FILE))
    ...
    for k, cast in enumerate(medatlas["local_cdi_id"]):
        ...
        with open(CDI_TEMPLATE, "r", encoding="utf8") as f:
            ...
            template = template.replace("CUSTODIAN_CODE", csr['custodian_code'])
            template = template.replace("CUSTODIAN_NAME", csr['custodian_name'])
            template = template.replace("CUSTODIAN_CONTACT", csr['custodian_contact'])
            
            template = template.replace("ORIGINATOR_CODE", csr['originator_code'])
            template = template.replace("ORIGINATOR_NAME", csr['acquisition_center'])
            template = template.replace("ORIGINATOR_CONTACT", csr['originator_contact'])
            ...
            #An "UNKNOWN" project was registered for IEO in SDN platform with code 12509
            if csr["custodian_code"] == "353" and csr["project_code"] == "":
                template = template.replace("PROJECT_CODE", "12509")
                template = template.replace(
                        "PROJECT_DESCRIPTION", 
                        "UNKNOWN {acronym=&quot;UNKNOWN&quot; organisation=&quot;IEO/ Spanish Oceanographic Institute&quot; country=&quot;Spain&quot;}"
                )                
            else:               
            ...
            # Make a pretty XML. First skip unicode declaration and parse it (
            # this removes also initial comments in xml) and convert again to
            # string in a pretty way
            template = template.split("\n",1)[1]
            # Problem when ampersand found in text
            template_pretty = ET.fromstring(template.replace('&','&#38;'))
            template_pretty = ET.tostring(template_pretty, pretty_print=True).decode('utf8')
            template_pretty = '<?xml version="1.0" encoding="UTF-8"?>\n' + \
                              '<!-- this file has been created with ctdcheck.ieo.es based on MIKADO version 3.6.2 -->\n' + \
                              template_pretty.rstrip("\n").rstrip()    
                                                  
            output_file = Path(outpath_cdi, cast + ".xml")
            with open(output_file, "w", encoding="utf8", newline="") as outfile:
                outfile.write(template_pretty)
