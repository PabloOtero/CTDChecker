...
@register_series_method
@register_dataframe_method
def location_at_sea(df, metadata):
    if (
        ("lat" not in metadata)
...
def instrument_range(df, var, cfg):
    ...
    flag[ma.getmaskarray(df[var])] = FLAG_MISSING
...
def gradient(df, var, cfg):
    ...
    flag[ma.getmaskarray(x)] = FLAG_MISSING
...
def digit_roll_over(df, var, cfg):
    ...
    flag = np.zeros(y.shape, dtype="i1")
    flag[ma.getmaskarray(x)] = FLAG_MISSING
    flag[np.nonzero(ma.absolute(y) > threshold)] = FLAG_BAD
    flag[np.nonzero(ma.absolute(y) <= threshold)] = FLAG_GOOD
...
def density_inversion_test(df):
    ...
    if "PRES" and "PSAL" in df.columns:
        t = df["TEMP"].values
        s = df["PSAL"].values
        ...
        flag = np.zeros(y.shape, dtype="i1")
        flag[np.nonzero(y < -threshold)] = FLAG_PROBABLY_BAD
        flag[np.nonzero(y >= -threshold)] = FLAG_GOOD
